...
#define BOARD_TYPE 3              //тип платы часов(0 - IN-12 (индикаторы стоят правильно) | 1 - IN-12 turned (индикаторы перевёрнуты) | 2 - IN-14 (обычная и неоновая точка) | 3 - другие индикаторы(платы)(настройка в config.h))
...
#define GEN_HV_VCC 150            //напряжение питания ламп(только для ОС на АЦП)(150..190)(в)
#define GEN_HV_R_HIGH 640         //сопротивление верхнего плеча резисторного делителя(только для ОС на АЦП)(кОм) 676
#define GEN_HV_R_LOW 9.83           //сопротивление нижнего плеча резисторного делителя(только для ОС на АЦП)(кОм)
...
#define DEFAULT_KNOCK_SOUND 0      //звук нажатия клавиш(озвучки) по умолчанию(для бузера)(0 - звук выключен | 1 - звук включен)(для плеера)(0 - озвучка выключена | 1 - озвучка включена)
...
#define DEFAULT_NIGHT_START 8      //час перехода на ночную подсветку по умолчанию(BRIGHT_N)(NIGHT_START = NIGHT_END - только дневная подсветка)
#define DEFAULT_NIGHT_END 8        //час перехода на дневную подсветку по умолчанию(BRIGHT)

//Настройки яркости индикаторов
#define DEFAULT_INDI_BRIGHT 30     //яркость цифр дневная по умолчанию(5..30)
#define DEFAULT_INDI_BRIGHT_N 5    //яркость цифр ночная по умолчанию(5..30)

//Настройки подсветки
#define DEFAULT_BACKL_BRIGHT 100   //яркость подсветки ламп дневная по умолчанию(10..250)(шаг 10)
#define DEFAULT_BACKL_BRIGHT_N 10  //яркость подсветки ламп ночная по умолчанию(0..250)(0 - подсветка выключена)(шаг 10)
#define BACKL_MIN_BRIGHT 10        //минимальная яркость подсветки ламп(10..120)
#define BACKL_REVERSE 1            //реверс порядка светодиодов WS2812B(0 - нормальное отображение | 1 - реверсивное отображение)(только для светодиодов WS2812B)
...
#define DEFAULT_HOUR_SOUND_START 23 //время начала звука смены часа по умолчанию(0..23)(ч)(DEFAULT_HOUR_SOUND_START = DEFAULT_HOUR_SOUND_END - выключить звук)
